...
struct channel_allinfo_st
{
    ...
    int pos;//用来打开文件中的第一个音乐文件 不行再下一个
...
struct channel_allinfo_st channel[MAX_CHANEL_ID+1];  //用数组存放所有的节目单信息   +1 是因为0为节目单
...
static struct channel_allinfo_st * path2entry(const char *dir)
{
    ...
    strcat(pathall,dir); //将dir（/var/media/ch1）追加到pathall上
...
int mlib_getchannel_list(struct medialib_des_st **result,int *resnum)//结构体指针数组，去存放相应的节目单信息
{
    ...
    snprintf(path,PATHSIZE,"%s/*",server_conf.file_lib);  //打印媒体库的位置   DEFAULT_FILE_LIB ="/var/media"
    if(glob(path,0,NULL,&glob_res))  //解析path内的路径 结果放在glob_res
    {
        return -1;
    }
    ...
    for(i=0;i<glob_res.gl_pathc;i++)   //开始解析路径的内容
    {
        //globres.gl_pathv[i] -> "/var/media/ch1"
        allinfo = path2entry(glob_res.gl_pathv[i]);  //glob_res.gl_pathv[i] --每个待解析的路径 用allinfo解析glob_res.gl_pathv[i] 
        if(allinfo!=NULL)
        {//成功解析文件
